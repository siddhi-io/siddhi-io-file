...
/**
 * This extension can be used to move a file from a particular source to a destination.
 */
@Extension(
        name = "move",
        namespace = "file",
        description = "This function moves a particular file from a source to a given destination",
        parameters = {
                @Parameter(
                        name = "file.source.path",
                        description = "The source file path of the file to be moved.",
                        type = DataType.STRING
                ),
                @Parameter(
                        name = "file.destination.path",
                        description = "The destination file path of the file to be moved.",
                        type = DataType.STRING
                )
        },
        returnAttributes = {
                @ReturnAttribute(
                        description = "The success of the file move.",
                        type = DataType.BOOL
                )
        },
        examples = {
                @Example(
                        syntax = "from MoveFileStream\n" +
                                "select file:move('/User/wso2/source/test.txt', 'User/wso2/destination/') as moved\n" +
                                "insert into  ResultStream;",
                        description = "This query moved a particular file to a given path. " +
                                "The successfulness of the process will be returned as an boolean to the" +
                                "stream named 'RecordStream'."
                )
        }
)
public class FileMoveExtension extends FunctionExecutor {
    ...
    protected StateFactory init(ExpressionExecutor[] attributeExpressionExecutors, ConfigReader configReader,
        ...
        if (executor1.getReturnType() != STRING) {
            throw new SiddhiAppValidationException("Invalid parameter type found for the first argument of "
                    + "file:move() function, required " + STRING.toString() + ", but found "
    ...
    protected Object execute(Object[] data, State state) {
        ...
        try {
            ...
            vfsClientConnectorCallback.waitTillDone(5000, sourceFileUri);
        } catch (ClientConnectorException e) {
            throw new SiddhiAppRuntimeException("Failure occurred in vfs-client while moving the file " +
                    sourceFileUri + ".", e);
        } catch (InterruptedException e) {
            throw new SiddhiAppRuntimeException("Failed to get callback from vfs-client  for file " + sourceFileUri +
                    ".", e);
        }
        return true;
    }
