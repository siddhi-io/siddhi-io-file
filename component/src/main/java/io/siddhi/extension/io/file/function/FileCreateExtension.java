...
/**
 * This extension can be used to create a file.
 */
@Extension(
        name = "create",
        namespace = "file",
        description = "This function create a file in the given location",
        parameters = {
                @Parameter(
                        name = "file.path",
                        description = "The file path which needs to be created.",
                        type = DataType.STRING
                )
        },
        returnAttributes = {
                @ReturnAttribute(
                        description = "The success of the file creation.",
                        type = DataType.BOOL
                )
        },
        examples = {
                @Example(
                        syntax = "from CreateFileStream\n" +
                                "select file:create('/User/wso2/source/test.txt') as created\n" +
                                "insert into  ResultStream;",
                        description = "This query creates a particular file in the given path. " +
                                "The successfulness of the process will be returned as an boolean to the" +
                                "stream named 'RecordStream'."
                )
        }
)
public class FileCreateExtension extends FunctionExecutor {
    ...
    protected StateFactory init(ExpressionExecutor[] attributeExpressionExecutors, ConfigReader configReader,
        ...
        if (executor1.getReturnType() != STRING) {
            throw new SiddhiAppValidationException("Invalid parameter type found for the fileSourcePath " +
                    "(first argument) of file:create() function, required " + STRING.toString() + ", but found "
    ...
    protected Object execute(Object data, State state) {
        ...
        String fileSourcePath = (String) data;
        ...
        properties.put(Constants.ACTION, Constants.CREATE);
        try {
